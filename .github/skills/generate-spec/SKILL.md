...
### QA / Automatización
- [ ] Generar escenarios Gherkin (`/gherkin-case-generator`)
- [ ] Identificar riesgos (`/risk-identifier`)
- [ ] Implementar `.feature` files Karate (`/unit-testing`)
- [ ] Generar `pom.xml`, `karate-config.js` y `TestRunner.java` (`/unit-testing`)
- [ ] Ejecutar suite y validar resultados (`mvn test`)
```
